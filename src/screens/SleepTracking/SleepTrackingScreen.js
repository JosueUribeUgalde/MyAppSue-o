...
// ¡Sincronizado! Importamos tu función de recomendaciones diarias
import { getDailyRecommendation } from '../Tips/sleepRecommendations';
...
const SleepTrackingScreen = ({ navigation }) => {
    ...
    const handleSaveSleep = async () => {
        try {
            ...
            // --- CÁLCULO DEL TIP DIARIO ASOCIADO AL REGISTRO ---
            let diffMs = wakeTime.getTime() - bedTime.getTime();
            if (diffMs < 0) {
                diffMs += 24 * 60 * 60 * 1000; // Sumar 24h si pasó la medianoche
            }
            const horasNumericas = diffMs / (1000 * 60 * 60); // Conversión a flotante (ej. 7.5)

            const metaSueno = 8; // Meta ideal estándar
            const tipDelDia = getDailyRecommendation(horasNumericas, metaSueno);
            // --------------------------------------------------
            ...
            const datosDelFormulario = {
                ...
                notas: notes,
                recomendacion_tip: tipDelDia // <-- Guardamos la recomendación persistida
            };

            console.log('Guardando datos con recomendación integrada:', datosDelFormulario);
