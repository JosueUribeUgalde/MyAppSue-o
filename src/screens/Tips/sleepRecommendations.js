...
export const getDailyRecommendation = (horasTotalesString, sleepGoal = 8) => {
    ...
    if (difference <= -4) {
        return '¡Cuidado! Te quedaste muy lejos de tu meta de ${sleepGoal} horas (dormiste solo ${hoursRested.toFixed(1)}h). Para mejorar tu descanso, intenta establecer una rutina de sueño consistente, evitando las pantallas al menos 30 minutos antes de acostarte y creando un ambiente relajante en tu habitación.'; 
        if (difference <= -2}
        return `Te quedaste bastante lejos de tu meta de ${sleepGoal} horas (durmiste ${hoursRested.toFixed(1)}h). Hoy evita las pantallas pesadas antes de acostarte y reduce la cafeína por la tarde para ayudar a tu cuerpo a recuperarse.`;
    }
    if (difference < 0) {
    ...
    return `Dormiste ${hoursRested.toFixed(1)}h, superando por bastante tu meta de ${sleepGoal} horas. El exceso de sueño a veces causa pesadez; intenta activarte temprano dando una caminata bajo la luz solar.`;
};